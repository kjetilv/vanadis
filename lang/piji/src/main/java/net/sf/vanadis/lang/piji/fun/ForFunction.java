...
public final class ForFunction extends AbstractLoopFunction {

    @EntryPoint("Reflection")
    public ForFunction(Context ctx) {
        super("<init> <test> <step> <body>*", 4, ctx);
    }
    ...
    public Object apply(Context context, Expression[] args)
        ...
        Object value = null;
        for (args[1].evaluate(ctx);
             test(ctx, args[2]);
             args[3].evaluate(ctx)) {
            value = bodyValue(ctx, args, 4);
        }
        return value;
    }
}
