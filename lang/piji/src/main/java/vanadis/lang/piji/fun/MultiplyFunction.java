...
public final class MultiplyFunction extends AbstractFunction {

    @EntryPoint("Reflection")
    public MultiplyFunction(Context ctx) {
        super("Arithmetic multiplication", true, 1, ctx);
    }
    ...
    public Object apply(Context context, Expression[] args)
        ...
        PrimitiveNumberHolder value = DataHolderFactory.holder(1);
        for (int i = 1; i < args.length; i++) {
            Object object = args[i].evaluate(context);
            if (object instanceof PrimitiveNumberHolder) {
                value = value.mul((PrimitiveNumberHolder) object);
            } else {
